...
class CodeGenerator(vibelangVisitor):
    ...
    def visitVarAssign(self, ctx: vibelangParser.VarAssignContext) -> object:
        ...
        fields = [n.getText() for n in id_nodes] if first_token == 'self' else [n.getText() for n in id_nodes[1:]]  # pyright: ignore[reportOptionalIterable, reportGeneralTypeIssues, reportIndexIssue, reportOptionalSubscript]
    ...
    def visitIfStmt(self, ctx: vibelangParser.IfStmtContext):
        ...
        for i in range(ctx.getChildCount()):
            ...
            if child.getText() == 'else':
                in_else = True  # everything after this keyword belongs to the else-branch
    ...
    def visitRelExpr(self, ctx: vibelangParser.RelExprContext):
        if self.builder is None:
            raise SemanticError("Semantic error: Cannot allocate memory.", ctx.start.line)  # pyright: ignore[reportOptionalMemberAccess]
        ...
        if left_val is None or right_val is None:
            raise SemanticError("Semantic error: Invalid operand in relational expression.", ctx.start.line)  # pyright: ignore[reportOptionalMemberAccess]
        ...
        try:
            ...
            raise SemanticError(f"Semantic error: Unsupported relational operator '{operator}'.", ctx.start.line)  # pyright: ignore[reportOptionalMemberAccess]
    ...
    def visitCallExpr(self, ctx: vibelangParser.CallExprContext) -> object:
        ...
        if args_ctx is not None:
            ...
            for i, expr_ctx in enumerate(exprs):  # pyright: ignore[reportArgumentType]
                ...
                val = self.cast_to(val, expected_type)  # implicit cast to the parameter type  # pyright: ignore[reportArgumentType]
    ...
    def visitClassDefinition(self, ctx: vibelangParser.ClassDefinitionContext) -> object:
        ...
        for member_ctx in ctx.classMember():  # pyright: ignore[reportOptionalIterable, reportGeneralTypeIssues]
            if isinstance(member_ctx, vibelangParser.ClassFieldContext):  # a field (data member)
    ...
    def get_pointer_and_class(self, ctx) -> tuple[ir.Value, str]:  # pyright: ignore[reportUnknownParameterType, reportMissingParameterType]
        ...
        if isinstance(ctx, vibelangParser.IdExprContext):
            ...
            if field_name not in struct_fields:
                raise SemanticError(f"Semantic error: Field '{field_name}' not found.", ctx.start.line)  # pyright: ignore[reportOptionalMemberAccess]
